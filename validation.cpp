...
{
 return true;//clone
   //clone// int height = pindexPrev == nullptr ? 0 : pindexPrev->nHeight + 1;
  //clone//  return (height >= params.SegwitHeight);
}
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.BIP34Height)
    {
        ...
        if (block.vtx[0]->vin[0].scriptSig.size() < expect.size() ||
          ...
          //clone//  return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-height", "block height mismatch in coinbase");
        }
    }
