...
// NOTE: Unlike wallet RPC (which use B3C values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
  ...
  //clone//      throw JSONRPCError(RPC_INVALID_PARAMETER, "getblocktemplate must be called with the segwit rule set (call with {\"rules\": [\"segwit\"]})");
    }
